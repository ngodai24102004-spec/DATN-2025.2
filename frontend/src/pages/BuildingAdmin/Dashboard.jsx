                ...
                {loading ? (
                    ...
                    <div style={{ overflowX: 'auto' }}>
                        <div style={{ display: 'flex', alignItems: 'flex-start', minWidth: '1500px', gap: '4px', position: 'relative', zIndex: 1, paddingBottom: '10px' }}>
                            <div className="cyber-zone" style={{ flex: 1 }}><div className="cyber-zone-label">Heat Rejection / Tower</div><div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>{towers.length === 0 ? <p className="cyber-empty">— TRỐNG —</p> : towers.map(d => <DeviceCard key={d.id} device={d} icon={Fan} />)}</div></div>
                            <FlowArrow />
                            <div className="cyber-zone" style={{ flex: 1 }}><div className="cyber-zone-label">Condenser Pumps</div><div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>{coolingPumps.length === 0 ? <p className="cyber-empty">— TRỐNG —</p> : coolingPumps.map(d => <DeviceCard key={d.id} device={d} icon={Power} />)}</div></div>
                            <FlowArrow />
                            <div className="cyber-zone cyber-zone-chiller" style={{ flex: 1 }}><div className="cyber-zone-label">Chiller Plant</div><div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>{chillers.length === 0 ? <p className="cyber-empty">— TRỐNG —</p> : chillers.map(d => <DeviceCard key={d.id} device={d} icon={Snowflake} />)}</div></div>
                            <FlowArrow />
                            <div className="cyber-zone" style={{ flex: 1 }}><div className="cyber-zone-label">Chilled Water Pumps</div><div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>{coldPumps.length === 0 ? <p className="cyber-empty">— TRỐNG —</p> : coldPumps.map(d => <DeviceCard key={d.id} device={d} icon={Power} />)}</div></div>
                            <FlowArrow />
                            <div className="cyber-zone cyber-zone-pipe" style={{ flex: 1 }}><div className="cyber-zone-label">Telemetry / Pipes</div><div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>{pipes.length === 0 ? <p className="cyber-empty">— TRỐNG —</p> : pipes.map(d => <DeviceCard key={d.id} device={d} icon={Droplets} />)}</div></div>
                            <FlowArrow />
                            <div className="cyber-zone" style={{ flex: 1 }}><div className="cyber-zone-label">Isolation Valves</div><div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>{valves.length === 0 ? <p className="cyber-empty">— TRỐNG —</p> : valves.map(d => <DeviceCard key={d.id} device={d} icon={Activity} />)}</div></div>
                            <FlowArrow />
